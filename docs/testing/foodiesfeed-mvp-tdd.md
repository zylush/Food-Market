...
```text
corepack pnpm test:coverage  -> 101 tests passed; 95.72% statements, 89.70% branches, 93.60% functions, 95.72% lines
...
The `foodiesfeed-api` and `foodiesfeed-web` Vercel projects are live. Direct API health, same-origin rewritten health, the manifest, and all four locale shells return successfully; the API response identifies the Frankfurt `fra1` region. Live browser QA covered real Open Food Facts search, public/private response separation, recent-search persistence, locale cookies and document languages, keyboard traversal, and responsive layouts. The Stripe test product, recurring EUR price, and six-event webhook endpoint are configured. One explicitly approved sandbox Checkout completed successfully and exposed a Vercel raw-body integration gap before entitlement reconciliation; the regression fix is locally verified and awaits deployed webhook verification. Active-entitlement/nutrition verification and subsequent test-subscription cancellation remain unclaimed at this checkpoint.
